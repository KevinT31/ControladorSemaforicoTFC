...
async def root():
    ...
    interfaz_path = Path(__file__).parent.parent / "interfaz_web" / "index.html"
    if not interfaz_path.exists():
        interfaz_path = Path(__file__).parent.parent / "interfaz-web" / "index.html"
    if interfaz_path.exists():
        return FileResponse(interfaz_path)
    return {"mensaje": "Sistema de Control Semafórico Activo"}
...
interfaz_path = Path(__file__).parent.parent / "interfaz_web"
if not interfaz_path.exists():
    interfaz_path = Path(__file__).parent.parent / "interfaz-web"

if interfaz_path.exists():
    app.mount("/static", StaticFiles(directory=str(interfaz_path)), name="static")
